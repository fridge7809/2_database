...
FROM SPORTS S
ORDER BY S.NAME ASC -- Task 2

SELECT S.NAME AS ONE_RESULT
FROM SPORTS S
JOIN RESULTS R ON R.SPORTID = S.ID
GROUP BY S.NAME -- task 3 number of athletes that competed in 1 sport

SELECT COUNT(DISTINCT R.PEOPLEID) AS ATHELETES_COMPETED
FROM RESULTS R -- task 4 The ID and name of athletes who have at least twenty results.

SELECT P.ID, P.NAME
FROM PEOPLE P
JOIN RESULTS R ON R.PEOPLEID = P.ID
GROUP BY P.ID, P.NAME
HAVING COUNT(R.PEOPLEID) >= 20 -- task 5 The ID, name and gender description of all athletes that currently hold a record.
...
WHERE R.RESULT = S.RECORD
GROUP BY S.ID,
	R.PEOPLEID -- Task 7 convert floating point to double digit precision
...
HAVING COUNT(*) >= 20 -- Task 8 extract year from date
...
WHERE C.PLACE like 'Hvide Sande'
	AND DATE_PART('year',
						C.HELD) = 2009
GROUP BY P.ID,
	G.DESCRIPTION -- Task 9 string splitting
